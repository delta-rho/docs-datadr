...
This tutorial covers an implementation of Divide and Recombine (D&R) in the R statistical programming environment, via an R package called `datadr`.  This is one component of the [Tessera](http://tessera.io) environment for the analysis of large complex data.
...
Suppose we want a more meaningful global summary, such as computing quantiles.  `datadr` can do this in a division-independent way with `r rdl("drQuantile()")`.  For example, let's look at quantiles for the median list price and plot them using `r rdl("xyplot()")` from the `lattice` package:
