...
This tutorial covers an implementation of Divide and Recombine (D&R) in the R statistical programming environment, an R package called `datadr`.  This is one component of the [Tessera](http://tesseradata.org) environment for the analysis of large complex data.
...
References:
   - [tesseradata.org](http://tesseradata.org)
...
The example we go through will be a small dataset that we can handle in a local R session, and therefore we only need to have these two packages installed.  For other installation options when dealing with larger data sets, see the [quickstart](http://tesseradata.org/#quickstart) on our website.
...
```{r quickstart_loadpackages, eval=TRUE, echo=TRUE}
library(housingData)
library(datadr)
library(trelliscope)
...
This applies `lmCoef()` to each subset in a deferred fashion, meaning that for all intents and purposes we can think of `byCountySlope` as a distributed data object that containes the result of `lmCoef()` being applied to each subset.  But computation is deffered until another data operation is applied to `byCountySlope`, such as a recombination, which we will do next.
...
We have a more comprehensive tutorial for using [Trelliscope](http://tesseradata.org/docs-trelliscope/), but for completeness here and for some motivation to get through this tutorial and move on to the Trelliscope tutorial, we provide a simple example of taking a ddf and creating a Trelliscope display from it.
...
If you have not been following along but are wondering what that `view()` command did, you can visit <a href="http://hafen.shinyapps.io/housingDemo/" target="_blank">here</a> for an online version.  You will find a list of displays to choose from, of which the one with the name `list_sold_vs_time` is the one we just created.  This brings up the point that you can share your Trelliscope displays online -- more about that as well as how to use the viewer will be covered in the Trelliscope tutorial -- but feel free to play around with the viewer.

This covers the basics of `datadr` and a bit of `trelliscope`.  Hopefully you now feel comfortable enough to dive in and try some things out.  The remainder of this tutorial and the [Trelliscope](http://tesseradata.org/docs-trelliscope/) tutorial will provide greater detail.

### For plyr / dplyr Users ###

Now that we have seen some examples and have a good feel for what `datadr` can do, if you have used `plyr` or `dplyr` packages, you may be noticing some similarities.  
...
It is also worth discussing some of the similarites and differences to help understand when `datadr` is useful.  We expand on this in the [FAQ](#faq).
